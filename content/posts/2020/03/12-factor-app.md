...
1）使用统一的规范，可以使新进开发人员节省时间成本，按照最佳实践走即可；
  
2）应用应与底层操作系统解耦，以在各种运行环境之间提供最大的可移植性；
  
3）应用应适于部署在现代化云平台上，摒弃对服务器和系统管理的依赖；
  
4）开发环境与生产环境之间不应有很多差异，可以以最大化的敏捷性进行持续部署；
  
5）可以在不对工具，架构，开发实践进行重大改变的情况下进行自由扩展。
...
a）连接数据库，缓存或其它后端服务的配置信息；
  
b）连接诸如AWS s3等外部服务的密钥信息；
...
b）易将配置文件以不同的格式放置在不同的地方，不便于统一管理；
...
<img class="aligncenter" src="/wp-content/uploads/2020/03/12-factor-app-backing-service.png" width="371" height="240" />
...
1）构建阶段是将代码仓库转换为一个可执行包
  
部署过程从代码的某次提交点拉一个版本出来，构建即是获取该版本的依赖并且将其编译为二进制资产。
  
2）发布阶段是将构建出来的包与当前部署配置相结合
  
发布阶段组合可以在运行环境立即执行的包与配置。
  
3）运行阶段即是将应用在运行环境运行起来
  
运行应用对应版本的进程。

十二因子指导原则严格将构建，发布，运行阶段分离。诸如，我们无法在运行阶段修改代码，因我们无法将这些变更传回到构建阶段。

<img class="aligncenter" src="/wp-content/uploads/2020/03/12-factor-app-build-run.png" width="431" height="121" />
...
<img class="aligncenter" src="/wp-content/uploads/2020/03/12-factor-app-concurrency.png" width="309" height="258" />
...
a）时间鸿沟，开发的代码可能很久才上线生产；
  
b）个人鸿沟，开发写代码，运维部署代码；
  
c）工具鸿沟，开发环境与生产环境使用的技术栈有差别（开发环境使用Nginx，MySQL，OS X；生产环境使用Apache，SQLite，Linux）。

十二因子应用建议设计时当考虑持续部署，将开发与生产的差别保持的越小越好。再看上面的3个鸿沟：
  
a）解决时间鸿沟，开发者开发了代码，几分钟即部署到生产；
  
b）解决个人鸿沟，DevOps打通，写代码的人要关注部署；
  
c）解决工具鸿沟，开发环境与生产环境越接近越好。
...
在测试及开发环境，每个进程的输出会被运行环境捕获，然后存档到某些位置以备查看。存档位置不应由应用来配置，而应交给运行环境。开源的日志路由（诸如Fluentd）即是做这些事情的。
...
c）按用户定义规则来进行告警（如每分钟错误数超过某阈值即告警）。
...
b）运行控制台任务以执行代码或对线上数据库作检查；
  
c）运行一次性脚本（如php scripts/fix\_bad\_records.php）。

一次性管理进程当与常规常驻进程使用一样的环境。即管理进程与其它进程使用相同的代码和配置，且管理代码随应用程序一起发布，从而规避不同步问题。

所有进程类型应使用相同的依赖隔离技术。如，使用\`bundle exec thin start\`运行ruby Web进程，使用\`bundle exec rake db:migrate执行数据迁移\`。

> 参考资料
  
> [1]&nbsp;<a href="https://en.wikipedia.org/wiki/Twelve-Factor_App_methodology" target="blank">https://en.wikipedia.org/wiki/Twelve-Factor_App_methodology</a>
  
> [2]&nbsp;<a href="https://12factor.net/" target="blank">https://12factor.net/</a>
