...
- `void addAtIndex(int index, int val)` 若index不合法（index < 0 || index >= list.len），直接退出；首先判断是否在头前新增，若是，则将新增节点的next指向现在的头，新的头指向新增的节点（若在空链表第一次新增，同时将尾也指向新增的节点）；其次，需要判断是否在尾部后面追加，若是，则将现在的尾的next指向新增节点，新的尾指向新增的节点；再次，若是在链表中间某处新增，则找到新增位置的前一个节点，将新增的节点插入并建立新的连接关系。最后将链表长度加1。
