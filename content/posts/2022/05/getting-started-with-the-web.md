...
- 整体请求顺序概述
  ...
  ② 浏览器向服务器发送一条 HTTP 请求消息，要求它向客户端发送一份网站副本。此消息以及客户端和服务器之间发送的所有其它数据均通过用户本地连接的 TCP/IP 发送。

  ③ 若服务器同意客户端的请求，则会向客户端发送“200 OK”消息，然后将网站的文件以一组小块的方式连续发送到浏览器。

  ④ 浏览器接到全部小块后，将其组装成一个完整的网页来显示给用户。
