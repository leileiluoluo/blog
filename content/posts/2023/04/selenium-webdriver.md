...
下面，开始对 WebDriver 进行初步使用。主要学习如何使用 WebDriver 创建浏览器对象、打开页面、定位元素、输入内容和点击按钮等操作。

用到的页面为 Selenium 官网提供的一个「[Web Form 示例页面](https://www.selenium.dev/selenium/web/web-form.html)」。

该页面包含文本输入框、下拉框、文件上传、日期选择等。页面截图如下：
...
如上 Python 代码即用到了 Selenium 包，运行代码前需要安装一下`selenium`模块。
...
下面总结一下，我们使用 WebDriver 的几个主要的方法。
...
- 等待页面元素出现
  ...
  可以使用`WebDriverWait`来等待某个页面元素出现。
