...
模式可用于以多种方式组织数据。安全模式最佳实践可防止不受信任的用户更改其他用户的查询行为。当数据库不使用安全模式时，希望安全查询该数据库的用户将在每个会话开始时采取保护操作。具体来说，他们将通过将`search_path`设置为空字符串或以其它方式从`search_path`中删除非超级用户可写的模式来开始每个会话。默认配置很容易支持一些最佳实践：
