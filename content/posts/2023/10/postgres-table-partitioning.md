...
约束排除是一种类似于分区裁剪的查询优化技术。虽主要用于继承式分区，但也可用于声明式分区。

约束排除的工作方式与分区裁剪非常相似，不同之处在于它使用的是表的`CHECK`约束，而分区裁剪使用的是表的分区边界。另一个区别是约束排除仅在计划时应用，不会在执行时尝试删除分区。

约束排除使用`CHECK`约束，这使得它与分区裁剪相比速度较慢，但有时却是一种优势：因为在声明式分区的表上也可以定义约束，因此除了其内部分区边界之外，约束排除可能能够从查询计划中省略其它分区。

`constraint_exclusion`推荐的设置既不是`on`也不是`off`，而是一个称为`partition`的中间设置，这样该技术仅应用于可能处理继承分区表的查询。`on`设置将使计划器检查所有查询（包括完全没必要做检查的简单查询）的`CHECK`约束。

此外，使用约束排除还有如下注意事项：

- 仅当查询的`WHERE`子句包含常量（或外部提供的参数）时，约束排除才有效。例如，约束排除无法优化与不可变函数（如`CURRENT_TIMESTAMP`）的比较，因为计划器无法知道函数值在运行时落入哪张子表。
...
应谨慎选择如何对表进行分区，因为糟糕的设计可能会对查询计划和执行的性能产生极大的影响。

最关键的部分是选择对数据进行分区的列，通常是从经常出现在对分区表执行查询的`WHERE`子句中的列选择分区列，也可能会因`PRIMARY KEY`或`UNIQUE`约束的要求而被迫做其它选择。
