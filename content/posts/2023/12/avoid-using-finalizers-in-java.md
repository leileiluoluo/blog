...
- 未捕获的异常会被忽略
  ...
  若 JVM 线程执行 Finalizer 对象的 `finalize()` 方法时抛出未捕获的异常，则该异常会被忽略（不会打印任何信息）且 `finalize()` 的执行将被终止。这样极有可能造成对象的状态异常，其它使用该对象的线程也可能出现异常行为。

- 性能问题

  对于 Finalizer 对象的 `finalize()` 方法，JVM 线程最多仅会调用一次。
