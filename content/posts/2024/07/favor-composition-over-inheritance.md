...
继承是指一个类（称为子类或派生类）可以从另一个类（称为父类或基类）继承其属性（数据）和方法（行为）的过程。子类可以重用父类的代码，并且可以在其基础上添加新的功能或修改现有功能。继承通过形成类之间的层次结构（也称为类的继承链）来组织和结构化代码。例如：有一个类 `Animal`，类 `Dog` 和 `Cat` 可以通过继承 `Animal` 类来拥有其属性和方法，同时也可以添加特定于 `Dog` 和 `Cat` 的行为。
