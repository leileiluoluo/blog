...
keywords:
  ...
  - 数据区域
  - 划分
description: 本文重点关注 Java 虚拟机运行时数据区域是如何划分的，然后重点对栈和堆这两块区域从分配和功用上进行一下对比。
---
...
- 堆既可以被实现成固定大小的，也可以是可扩展的，不过当前主流的 Java 虚拟机都是按照可扩展来实现的（通过参数 `-Xmx` 和 `-Xms` 设定）。如果在堆中没有内存来完成实例分配，并且堆也无法再扩展时，Java 虚拟机会抛出 `OutOfMemoryError` 异常。
