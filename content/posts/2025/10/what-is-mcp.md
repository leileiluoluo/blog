...
最后，我个人对 MCP 的理解是，抛去其与本地程序交互的 Stdio 之外，其与 RESTful API 类似，都是基于 HTTP 的一种 API 设计规范。不同的是，RESTful API 是给人调用的，且只给了一个参考框架，没有对数据体的设计细节进行统一和标准化；而 MCP 服务是给各个大语言模型调用的，所以需要以统一的方式告知所拥有的资源和对应的使用方法，标准化的数据体就是资源和其说明文档的集合体。
