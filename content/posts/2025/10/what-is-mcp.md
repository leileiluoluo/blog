...
类似于我们电脑的 USB-C 插口，使各种电子设备均能以统一的方式连接到电脑主机一样，MCP 使各种大语言模型聊天窗（如 ChatGPT、Claude Desktop）、各种代码编辑器（如 VS Code、Cursor）和各种 AI 应用等都能以标准的方式访问本地或远程应用程序。
...
可以看到，除了 C/S 架构中原有的 Client 和 Server 之外，MCP 多了一个 Host 的概念。这个 Host 就是 ChatGPT、Claude Desktop、Cursor 等大语言模型应用程序，用于管理和协调多个 MCP Client。
...
数据层除了通用的请求响应交互方式之外，还支持 MCP Client 和 MCP Server 之间基于 JSON-RPC 2.0 进行无响应体的消息通知。这种通信方式很适合用在 MCP Server 的特性或工具发生变更时通知 MCP Client 的场景。

介绍完 MCP 的架构之后，下面以实例的方式看一下 MCP Client 和 MCP Server 是如何基于 MCP 数据层交互的。

## 3 MCP Server 与 MCP Client 数据层交互样例

在该部分，我们以接近实际样例的方式演示一下 MCP Server 与 MCP Client 在数据层交互的过程。这一部分也是对我们有意愿在下一步使用 MCP 的开发者来说最重要的一部分。
...
连接建立好了之后，MCP Client 可以向 Server 发送一个 `tools/list` 的请求来获取 Server 提供的工具列表，以为后面的使用做准备。
...
MCP Server 在收到请求后，返回可用的工具列表，并对各个工具的功能和使用方式进行详细的说明。
...
经过上一步，MCP Client 知道了 MCP Server 所拥有的工具之后。这一步，就可以选取其中的某个工具，然后尝试使用了。

下面，MCP Client 发送了一个调用 `weather_current` 工具的请求，并提供了需要的参数。
...
这样，经过这三步，一次完整的 MCP Client 与 MCP Server 的交互就结束了。而我们知道，MCP Client 是集成到大语言模型中的，所以如上调用完全可以以自然语言对话的方式隐式进行。所以一个「智能」的现象就产生了：「你在大语言模型对话框问一个城市当天的天气，大语言模型给你返回了结果」。而当我们以搭积木的方式将多个 MCP Server 集成到大语言模型中时，就可以实现更多的「智能」了。

## 4 小结

本文首先以大家熟知的大语言模型为引子，引出了大语言模型的局限性，然后引出了 MCP 提出的缘由。接着，介绍了 MCP 的概念和架构，最后以实例的方式演示了 MCP Server 与 MCP Client 如何在数据层交互。

总结一下，我个人对 MCP 的理解是：抛去其与本地程序交互的 Stdio 之外，其与 RESTful API 类似，都是基于 HTTP 的一种 API 设计规范。不同的是，RESTful API 是给人调用的，且只给了一个参考框架，没有对数据体的设计细节进行统一和标准化；而 MCP 服务是给各个大语言模型调用的，所以需要以统一的方式告知所拥有的资源和对应的使用方法，标准化的数据体就是资源和其说明文档的集合体。
