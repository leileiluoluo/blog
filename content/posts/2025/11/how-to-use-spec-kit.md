...
下面我们将 Spec Kit 生成的、与项目 `spec-kit-demo` 同一级的 `specs` 文件夹拷贝到 `spec-kit-demo` 的 `.specify` 目录下。
...
可以看到，代码结构、包管理文件 `package.json` 、打包配置文件 `vite.config.ts` 等先被自动创建；接着，`main.tsx`、`App.tsx`、各个页面组件、可服用组件、工具类等被依次创建。
