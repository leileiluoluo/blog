...
那么这个指导 AI 编码助手的规范长什么样呢？经初步调查，目前业界推出的「规范驱动开发」的工作流基本都包含三个部分：需求 -> 设计 -> 实现（或任务）。即让 AI 编码助手着手编码前需要用文档的方式告知它业务需求、概要设计和实现步骤。目前业界所使用的指导 AI 做任务的「规范驱动开发」文档均由 Markdown 格式编写。
